...
const work = path.join(root, "outputs", "compare_200_560_work");
...
function norm(value = "") {
  return String(value).toLowerCase().normalize("NFD").replace(/\p{Diacritic}/gu, "").replace(/đ/g, "d")
    .replace(/\b(ky luc gia|klg|ong|ba|anh|chi|gs|pgs|ts|ths|bs|nghe nhan|nghe si|tien si|thac si|phap danh)\b/g, " ")
...
const raw200 = rows("sheet2.xml", strings).filter((r)=>r.row>2).map((r)=>({ row:r.row, name:String(r.values.get(7)||"").trim(), link:String(r.values.get(6)||"").trim(), gab:gabKey(r.values.get(6)||""), variants:nameVariants(r.values.get(7)||"") })).filter((r)=>r.name);
...
const table=[];
table.push(row(["KẾT QUẢ ĐỐI CHIẾU NGƯỜI TRONG TAB 200 VỚI DANH SÁCH 560"],1,xfsStart,28));
table.push(row([`Tổng ${results.length} người | Có trong 560: ${counts.yes} | Cần kiểm tra: ${counts.review} | Không tìm thấy: ${counts.no}`],2,xfsStart+1,24));
table.push(row(["Xanh = có trong 560; vàng = tên gần giống cần xác nhận; đỏ = chưa tìm thấy. Ưu tiên kết quả khớp bằng link GAB."],3,xfsStart+1,24));
table.push(`<row r="4" ht="8" customHeight="1"/>`);
const headers=["STT","TÊN TRONG TAB 200","KẾT QUẢ","TÊN TƯƠNG ỨNG TRONG 560","LINK GAB TAB 200","LINK GAB TRONG 560","CĂN CỨ ĐỐI CHIẾU","GHI CHÚ"];
table.push(row(headers,5,xfsStart,42));
results.forEach((r,i)=>{
  const statusStyle=r.status.startsWith("CÓ ")?xfsStart+2:r.status==="CẦN KIỂM TRA"?xfsStart+3:xfsStart+4;
...
const last=results.length+5, widths=[7,30,28,34,48,48,31,52];
...
write(path.join(xlsx,"xl","worksheets","sheet3.xml"),sheet);

let wb=read(path.join(xlsx,"xl","workbook.xml"));
wb=wb.replace(/<sheet[^>]*name="KẾT QUẢ 200 vs 560"[^>]*\/>/g,"");
wb=wb.replace("</sheets>",`<sheet sheetId="3" name="KẾT QUẢ 200 vs 560" state="visible" r:id="rId6"/></sheets>`);
write(path.join(xlsx,"xl","workbook.xml"),wb);
let rel=read(path.join(xlsx,"xl","_rels","workbook.xml.rels"));
rel=rel.replace(/<Relationship[^>]*Id="rId6"[^>]*\/>/g,"").replace("</Relationships>",`<Relationship Id="rId6" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/worksheet" Target="worksheets/sheet3.xml"/></Relationships>`);
...
console.log(JSON.stringify({source, rows560:list560.length, rawRows200:raw200.length, uniquePeople200:results.length, ...counts},null,2));
