...
const work=path.join(root,"outputs","pull_43_achievements_work");
...
for(const r of overlaps){overlapNames.set(norm(r.initial),r);overlapNames.set(norm(r.dev),r);for(const link of[r.initialLink,r.devLink]){const key=gabKey(link);if(key)overlapGab.set(key,r);}}
...
const pulled=[];
for(const a of achievements){const g=gabKey(a.gab);let hit=g?overlapGab.get(g):null,basis="";if(hit)basis="Trùng link GAB";if(!hit){hit=overlapNames.get(norm(a.name));if(hit)basis="Trùng tên đã chuẩn hóa";}if(hit)pulled.push({...a,initial:hit.initial,dev:hit.dev,basis});}
...
const table=[];table.push(row(["THÀNH TỰU CỦA NHÓM TRÙNG BAN ĐẦU - 560 TRONG TAB 200 NGƯỜI ĐÃ CHUẨN"],1,xfStart,30));table.push(row([`Trong 43 người trùng Ban đầu - 560: tìm thấy ${people} người trong tab 200, tương ứng ${pulled.length} dòng thành tựu.`],2,xfStart+1,24));table.push(row(["Mỗi dòng dưới đây là một thành tựu được kéo nguyên thông tin từ tab 200 người đã chuẩn; ưu tiên khớp bằng link GAB."],3,xfStart+1,24));table.push(`<row r="4" ht="8" customHeight="1"/>`);
...
console.log(JSON.stringify({source,overlapPeople:overlaps.length,matchedPeople:people,achievementRows:pulled.length},null,2));
